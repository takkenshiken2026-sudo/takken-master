...
def articles_guide_href(*, here: str) -> str:
    ...
    if here in ("compare", "numbers", "mistakes", "field"):
        return "../../articles/index.html"
    return "../../articles/index.html"
...
def knowledge_hub_tabs_html(*, current: str, **hrefs: str) -> str:
    ...
    valid = {tab_id for tab_id, _ in HUB_TABS} | {"field"}
    ...
    note = (
        ...
        '<a href="' + html.escape(articles_guide_href(here="field" if current == "field" else current)) + '">試験ガイド</a>をご覧ください。'
        "タブから各コンテンツへ移動できます。"
        "</p>"
    )
    nav = (
